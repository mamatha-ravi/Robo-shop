...
SCRIPT_DIR=$PWD
START_TIME=$(date +%s)
MONGODB_HOST=mongodb.devops88s.online
MYSQL_HOST=mysql.devops88ss.online
...
TOTAL_EXE_TIME(){
    ...
    echo -e "$(date "+%Y-%m-%d %H:%M:%S") | $app_name executed in: $G $TOTAL_TIME seconds $N" | tee -a $LOGS_FILE
}